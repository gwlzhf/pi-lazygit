...
test("keeps independent baselines for local branches", async () => {
  ...
  const featureChanges = changes(change("feature.ts", "M"));
  const hashes = new Map<string, string | null>([
    [resolve(root, "main.ts"), "main-hash"],
    [resolve(root, "feature.ts"), "feature-hash"],
  ]);
  const store = new BaselineStore();
  const signal = new AbortController().signal;
  ...
  expect(store.get(root, "branch:main")?.headIdentity).toBe("branch:main");
  expect(store.get(root, "branch:feature")?.headIdentity).toBe("branch:feature");
});
